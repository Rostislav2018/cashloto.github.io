...
$site_commission_loto_room_6 = 10;
    $error = 0;
	...
	function up_users_num($db,$qx,$w)
{
	...
	$users_roulette_koll = mysql_query("SELECT * FROM `p_roulette_users_game_6` $qx ",$db) or die(mysql_error());
	...
	function up_users_stats($db)
{
	...
	$users_roulette_koll = mysql_query("SELECT * FROM `p_roulette_users_stat_6` ORDER BY `data` DESC LIMIT 10",$db) or die(mysql_error());
...
function up_bank($db)
{
global $site_commission_loto_room_6;
$query = mysql_query("select SUM(`sum_bet`) from `p_roulette_users_game_6`",$db) or die(mysql_error());
$result = mysql_fetch_array($query);
$real_bank = $result[0]+0;

$formula = $real_bank/100 * $site_commission_loto_room_6;
...
$update_check_finish = mysql_query("UPDATE `p_roulette_users_game_6` SET `check_finish`=`check_finish`+'1' WHERE `id`='1' LIMIT 1",$db) or die(mysql_error());
//ожидание
sleep(1);
$finish_num = mysql_query("SELECT `check_finish` FROM `p_roulette_users_game_6` WHERE `id`='1' ",$db) or die(mysql_error());
...
$query_users_roulette = mysql_query("SELECT * FROM `p_roulette_users_game_6`",$db) or die(mysql_error());
...
$query_user_winner = mysql_query("SELECT * FROM `p_roulette_users_game_6` WHERE `user_id`='{$winner_id}'",$db) or die(mysql_error());
...
$query2 = mysql_query("select SUM(`sum_bet`) from `p_roulette_users_game_6` WHERE `user_id` NOT IN ($winner_id)",$db) or die(mysql_error());
$result2 = mysql_fetch_array($query2);
$bank2 = $result2[0];


$formula = $bank2/100 * $site_commission_loto_room_6;
...
$add_ref_link_trek = mysql_query("INSERT INTO `p_roulette_users_stat_6` SET `user_id`='{$winner_id}',`user_login`='{$rov_winner['user_login']}',`avatar`='{$rov_winner['avatar']}',`data`='{$data_roulette}',`sum`='{$user_profit}',`sum_bet`='{$rov_winner['sum_bet']}',`percent`='{$rov_winner['percent']}'",$db) or die(mysql_error());
...
$cleaning_roulette_tab = mysql_query("TRUNCATE TABLE `p_roulette_users_game_6`",$db) or die(mysql_error());
...
function last_game_time_win($db,$q)
{
	...
	$query = mysql_query("select * from `p_roulette_users_stat_6` ORDER BY `id` DESC LIMIT 1",$db) or die(mysql_error());
	$result = mysql_fetch_array($query);
	return $result[$q];
}
