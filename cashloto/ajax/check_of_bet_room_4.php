<?session_start();
	...
	$users_roulette_koll = mysql_query("SELECT * FROM `p_roulette_users_game_4`",$db) or die(mysql_error());
	...
	elseif($sum_bet <1) {	
    $error = 10;	
    $oshibka = 'Минимальная сумма ставки 1 рубль!'; 	 
	}
	...
	$user_roulette_test = mysql_query("SELECT * FROM `p_roulette_users_game_4` WHERE `user_id`='{$_SESSION['ses_user']}'",$db) or die(mysql_error());
    $test_users_roulette = mysql_num_rows($user_roulette_test);
	
	if ($test_users_roulette == 0){

	$data_roulette = time();
	$add_users_roulette_tab = mysql_query("INSERT INTO `p_roulette_users_game_4` SET `user_id`='{$_SESSION['ses_user']}',`user_login`='{$name}',`sum_bet`='{$sum_bet}',`avatar`='{$vk_avatar_100}',`data`='{$data_roulette}'",$db) or die(mysql_error());
/*
	if($user_ref_set >0) {
	...
	*/
	...
	$add_finish_time  = mysql_query("UPDATE `p_roulette_users_game_4` SET `finish`='{$finish}'",$db) or die(mysql_error());
	}
	...
	$update_user_bet  = mysql_query("UPDATE `p_roulette_users_game_4` SET `sum_bet`=`sum_bet`+'{$sum_bet}' WHERE `user_id`='{$_SESSION['ses_user']}' ",$db) or die(mysql_error());
	/*
	if($user_ref_set >0) {
	$referer_percent = $sum_bet/100 * $ref_percent_bet;
	...
	*/
	...
	$update_user_percent  = mysql_query("UPDATE `p_roulette_users_game_4` SET `percent`='{$percent_update}' WHERE `user_id`='{$rov['user_id']}'",$db) or die(mysql_error());
